...
async function handleAntideleteCommand(sock, chatId, message, match) {
    ...
    if (!match) {
        const statusMsg = await sock.sendMessage(chatId, {
            text: '🛡️ *BENZO-MD ANTIDELETE SYSTEM* 🛡️\n\n🔍 *Checking current status...*'
        }, { quoted: message });
    ...
    if (match === 'on') {
        ...
        await sock.sendMessage(chatId, {
            text: `❌ *ANTIDELETE DISABLED* ❌\n\n` +
                  ...
                  `⭐ *Benzo-MD Protection Disabled* ⭐`,
            edit: statusMsg.key
        });
    } else {
        await sock.sendMessage(chatId, {
            text: `❌ *INVALID COMMAND* ❌\n\n` +
                  `🛡️ *BENZO-MD ANTIDELETE SYSTEM*\n\n` +
                  `📝 *Usage:*\n` +
    ...
    try {
        ...
        if (isViewOnce && mediaType && fs.existsSync(mediaPath)) {
            try {
                ...
                const mediaOptions = {
                    caption: `🕵️ *BENZO-MD VIEW-ONCE CAPTURED*\n\n` +
                            `👤 *Sender:* @${senderName}\n` +
                            `📱 *Number:* ${sender}\n` +
                            `📁 *Type:* ${mediaType.toUpperCase()}\n` +
                            `🔒 *Status:* View-Once Message\n\n` +
                            `🛡️ *Captured by Benzo-MD Anti-Delete*\n` +
                            `⭐ *Forwarded to Owner*`,
                    mentions: [sender]
                };
                if (mediaType === 'image') {
...
async function handleMessageRevocation(sock, revocationMessage) {
    try {
        ...
        const statusMsg = await sock.sendMessage(ownerNumber, {
            text: `🛡️ *BENZO-MD ANTIDELETE ALERT* 🛡️\n\n` +
        ...
        if (original.mediaType && fs.existsSync(original.mediaPath)) {
            const mediaOptions = {
                caption: `🛡️ *BENZO-MD RECOVERED MEDIA* 🛡️\n\n` +

...
async function xxxCommand(sock, chatId, message) {
    try {
        ...
        // Log for debugging
        console.log(`[BENZO-MD XXX] Received message: "${text}" in chat: ${chatId}`);
        
        // Check if message starts with .xxx command
        if (!text.startsWith('.xxx')) {
            console.log(`[BENZO-MD XXX] Message doesn't start with .xxx, ignoring`);
            return;
        }
...
╚═══════════════════════╝`;
        ...
        console.log(`[BENZO-MD XXX] Processing query: "${searchQuery}"`);
        ...
        try {
            ...
            console.log(`[BENZO-MD XXX] Could not send quoted message, sending regular`);
            statusMsg = await sock.sendMessage(chatId, { 
                text: `🔍 *BENZO-MD XXX BOT*\n\n*Searching:* "${searchQuery}"\n\n⏳ *Please wait while we process your request...*` 
            });
        }
        ...
        if (searchQuery.match(/^https?:\/\//i)) {
            ...
            isDirectUrl = true;
            console.log(`[BENZO-MD XXX] Direct URL detected: ${contentUrl}`);
            ...
            try {
                ...
                console.log(`[BENZO-MD XXX] Could not edit message, sending new`);
                await sock.sendMessage(chatId, {
                    text: `✅ *BENZO-MD XXX BOT*\n\n🔗 *Link Detected*\n📥 Processing premium content...\n\n⚡ *Getting download information...*`
                });
            }
            ...
            try {
                ...
                console.log(`[BENZO-MD XXX] Found ${searchResults.length} results`);
            } catch (error) {
                console.error(`[BENZO-MD XXX] Search error:`, error);
            ...
            console.log(`[BENZO-MD XXX] Selected content: ${contentTitle} (${contentUrl})`);
        ...
        try {
            ...
            console.log(`[BENZO-MD XXX] Found ${downloadLinks.length} download links`);
        } catch (error) {
            console.error(`[BENZO-MD XXX] Download error:`, error);
        ...
        if (!downloadLinks || downloadLinks.length === 0) {
            console.log(`[BENZO-MD XXX] No download links found for: ${contentUrl}`);
        ...
        console.log(`[BENZO-MD XXX] Selected download: ${bestQuality.url} (${bestQuality.quality})`);
...
╚═══════════════════════╝`;
        ...
        try {
            await sock.sendMessage(chatId, {
                ...
                mimetype: 'video/mp4',
                fileName: `BENZO-MD_${finalTitle.substring(0, 30).replace(/[^a-z0-9]/gi, '_')}.mp4`,
                caption: caption
            });
        } catch (sendError) {
            console.error(`[BENZO-MD XXX] Error sending video:`, sendError);
        ...
        console.log(`[BENZO-MD XXX] Command completed successfully for: "${searchQuery}"`);

    } catch (error) {
        console.error('[BENZO-MD XXX BOT] Error:', error);
...
╚═══════════════════════╝` 
            ...
            console.error(`[BENZO-MD XXX] Could not send error message:`, sendError);
        }
    }
}
